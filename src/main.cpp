...
static void shutdownAll() {
    ...
    auto safe = [&](const char* name, auto fn) {
        try   { fn(); log.info("[main] {} shutdown OK.", name); }
        catch (const std::exception& e) {
    ...
    safe("LicenseClient",     [&]{ if (g_licenseClient){ g_licenseClient->shutdown(); g_licenseClient.reset(); } });
    safe("PluginManager",     [&]{ if (g_plugins)      { g_plugins->shutdown();       g_plugins.reset(); } });
    safe("StreamBroadcaster", [&]{ if (g_broadcaster)  { g_broadcaster->shutdown();   g_broadcaster.reset(); } });
    safe("RTMPOutput",        [&]{ if (g_rtmp)         { g_rtmp->shutdown();          g_rtmp.reset(); } });
    safe("DiskSpaceMonitor",  [&]{ if (g_diskMonitor)  { g_diskMonitor->shutdown();   g_diskMonitor.reset(); } });
    safe("StreamRecorder",    [&]{ if (g_recorder)     { g_recorder->shutdown();      g_recorder.reset(); } });
    safe("VCamBridge",        [&]{ if (g_vcam)         { g_vcam->shutdown();          g_vcam.reset(); } });
    safe("AndroidControl",    [&]{ if (g_androidControl){ g_androidControl->shutdown(); g_androidControl.reset(); } });
    safe("KeyboardInput",     [&]{ if (g_keyboardInput){ g_keyboardInput->shutdown(); g_keyboardInput.reset(); } });
    ...
    safe("VideoDecoder",      [&]{ if (g_videoDecoder) { g_videoDecoder->shutdown();  g_videoDecoder.reset(); } });
    safe("MirrorWindow",      [&]{ if (g_mirrorWindow) { g_mirrorWindow->shutdown();  g_mirrorWindow.reset(); } });
    safe("DX12Manager",       [&]{ if (g_dx12)         { g_dx12->shutdown();          g_dx12.reset(); } });
    safe("VirtualAudio",      [&]{ if (g_virtualAudio) { g_virtualAudio->shutdown();  g_virtualAudio.reset(); } });
    safe("AudioMixer",        [&]{ if (g_audioMixer)   { g_audioMixer->shutdown();    g_audioMixer.reset(); } });
    safe("AudioLoopback",     [&]{ if (g_audioLoopback){ g_audioLoopback->shutdown(); g_audioLoopback.reset(); } });
    safe("FeatureGate",       [&]{ if (g_featureGate)  { g_featureGate->shutdown();   g_featureGate.reset(); } });
    safe("LicenseManager",    [&]{ if (g_license)      { g_license->shutdown();       g_license.reset(); } });
    safe("SecurityVault",     [&]{ if (g_vault)        { g_vault->shutdown();         g_vault.reset(); } });
...
int main(int argc, char* argv[]) {
    ...
    // Register app as a Windows Event Log source
    {
        ...
        RegCreateKeyExW(HKEY_LOCAL_MACHINE, regPath.c_str(), 0, nullptr,
    ...
    if (!aura::OSVersionHelper::version().isWindows10OrLater) {
        MessageBoxA(nullptr, "Windows 10 or later required.",
                    "Unsupported OS", MB_ICONERROR);
        aura::Logger::shutdown();
        return 1;
    }
    ...
    SAFE_INIT("VirtualAudioDriver", {
        ...
        if (g_audioMixer && g_virtualAudio) {
            g_audioMixer->addOutputSink([](const float* s, uint32_t n,
                ...
                if (g_virtualAudio) g_virtualAudio->writeSamples(s, n, sr, ch);
            });
        }
    });
    ...
    SAFE_INIT("VideoDecoder", {
        ...
        g_videoDecoder->setFrameCallback([](const aura::DecodedFrame& frame) {
            ...
            if (g_encoder && frame.texture) {
                g_encoder->feedFrame(frame.texture,
                    static_cast<int64_t>(frame.presentationTimeUs));
            }
            ...
            if (g_networkStats) {
                ...
                if (frame.wasDropped) g_networkStats->incrementDroppedFrames();
            }
        });
    });
    ...
    SAFE_INIT("NALParser", {
        g_nalParser = std::make_unique<aura::NALParser>(
            [](aura::NalUnit nal) {
                if (!g_videoDecoder) return;
                g_videoDecoder->submitNAL(
                    nal.data, nal.isKeyframe, 0 /*pts filled by demuxer*/);
    ...
    SAFE_INIT("ReconnectManager", {
        ...
        g_reconnect->setReconnectAttempt([] {
            ...
            if (g_videoDecoder)  g_videoDecoder->resetState();
            if (g_reorderCache)  g_reorderCache->flush();
            if (g_fec)           g_fec->reset();
    ...
    SAFE_INIT("ReceiverSocket", {
        ...
        auto packetCallback = [](aura::RawPacket pkt) {
            ...
            if (g_networkStats) {
                ...
                auto now = std::chrono::steady_clock::now();
                double interMs = std::chrono::duration<double, std::milli>(
                    now - prevArrival).count();
                prevArrival = now;
            ...
            g_reorderCache->drain([](aura::OrderedPacket ordered) {
                ...
                g_fec->feedPacket(ordered.sequenceNumber,
                                  isParity,
                                  std::move(ordered.payload));
            });
        };

        g_sharedSocket = std::make_shared<aura::ReceiverSocket>(
            kAirPlayVideoPort, packetCallback);
    ...
    SAFE_INIT("StreamRecorder", {
        ...
        if (g_audioMixer) {
            g_audioMixer->addOutputSink([](const float* samples, uint32_t numFrames,
                ...
                if (g_recorder) g_recorder->onAudioSamples(samples, numFrames,
                                                           sampleRate, channels);
            });
        }
    });
        ...
        g_encoder->setCallback([](const aura::EncodedPacket& ep) {
            if (g_rtmp)
                g_rtmp->sendVideoPacket(ep.data.data(), ep.data.size(),
                                        ep.pts, ep.isKeyframe);
            if (g_recorder)
                g_recorder->feedVideoPacket(ep.data.data(), ep.data.size(),
                                            ep.pts, ep.isKeyframe);
        });
    });
    ...
    SAFE_INIT("StreamBroadcaster", {
        g_broadcaster = std::make_unique<aura::StreamBroadcaster>(
            g_vcam.get(), g_recorder.get(), g_rtmp.get());
        g_broadcaster->init();
    });
    ...
    SAFE_INIT("AirPlay2Host", {
        ...
        g_airplay->setSessionEndedCallback([](const std::string& deviceId) {
            if (g_reconnect) g_reconnect->onDisconnected();
            ...
            if (g_audioLoopback) g_audioLoopback->stop();
            if (g_sharedSocket)  g_sharedSocket->clearAllowedSource();
            if (g_videoDecoder)  g_videoDecoder->resetState();
            if (g_reorderCache)  g_reorderCache->flush();
            if (g_fec)           g_fec->reset();
            if (g_nalParser)     g_nalParser->reset();
        });
        g_airplay->setPinRequestCallback([](const std::string& pin) {
        ...
        g_airplay->setPairingResultCallback([](bool success) {
            ...
            if (g_hubWindow) {
                ...
                QMetaObject::invokeMethod(
                    g_hubWindow.get(), [success]() {
                        auto* hm = g_hubWindow ? g_hubWindow->hubModel() : nullptr;
                        if (hm) hm->notifyPairingResult(success);
                    },
                    Qt::QueuedConnection);
            }
        });
    ...
    SAFE_INIT("ADBBridge", {
        ...
        g_adb->setPairingResultCallback([](const std::string& serial, bool success) {
            ...
            if (g_hubWindow) {
                QMetaObject::invokeMethod(g_hubWindow.get(), [success]() {
                    auto* hm = g_hubWindow ? g_hubWindow->hubModel() : nullptr;
                    if (hm) hm->notifyPairingResult(success);
                }, Qt::QueuedConnection);
            }
    ...
    SAFE_INIT("MDNSService", {
        ...
        g_mdns->init();
        const std::string displayName = g_settings
    ...
    SAFE_INIT("DeviceManager", {
        ...
        QObject::connect(g_deviceManager.get(), &aura::DeviceManager::scanRequested,
            []() {
                if (g_mdns) g_mdns->start(); // re-advertise on rescan
            });
        QObject::connect(g_deviceManager.get(), &aura::DeviceManager::mirrorRequested,
            [](const QString& deviceId) {
                if (g_adb) {
                    g_adb->startMirroring(
                        deviceId.toStdString(),
                        [](const uint8_t* data, size_t len, bool isKey) {
                            if (!g_nalParser || len == 0) return;
                            g_nalParser->feedPacket(
    ...
    SAFE_INIT("NetworkStatsModel", {
        ...
        if (g_bitratePID) {
            g_bitratePID->setOnBitrateChanged([](float bps) {
                if (g_networkStats) g_networkStats->setCurrentBitrateBps(bps);
            });
        }
    });
    ...
    SAFE_INIT("AirPlay2Host.start",       { if (g_airplay)      g_airplay->start(); });
    SAFE_INIT("CastV2Host.start",         { if (g_cast)         g_cast->start(); });
    SAFE_INIT("ADBBridge.start",          { if (g_adb)          g_adb->start(); });
    SAFE_INIT("NetworkStatsModel.start",  { if (g_networkStats) g_networkStats->start(); });
    ...
    SAFE_INIT("HubWindow", {
        ...
        if (g_settings) {
            ...
            QObject::connect(g_settings.get(), &aura::SettingsModel::uiChanged,
                             g_hubWindow.get(), []{
    ...
    if (g_hubWindow) {
        ...
        if (hub) {
            ...
            QObject::connect(hub, &aura::HubModel::mirroringStartRequested,
                             hub, []{
            ...
            QObject::connect(hub, &aura::HubModel::mirroringStopRequested,
                             hub, []{
            ...
            QObject::connect(hub, &aura::HubModel::pairingPinSubmitted,
                             hub, [](const QString& pin){
                if (!g_adb) return;
                const auto parts = pin.split('|');
                if (parts.size() == 2) {
                    const std::string addr = parts[0].toStdString();
                    const std::string code = parts[1].toStdString();
                    std::thread([addr, code]{
                        g_adb->pairWireless(addr, code);
                    }).detach();
                } else {
    ...
    if (splash) {
        splash->finish(g_hubWindow
            ? static_cast<QWidget*>(g_hubWindow.get()) : nullptr);
        delete splash;
        splash = nullptr;
    }
    ...
    if (g_telemetry && g_telemetry->isEnabled()) {
        g_telemetry->reportStartup(
            g_dx12 ? g_dx12->adapterName() : "Unknown GPU",
